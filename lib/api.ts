...
// CRITICAL FIX: Add this interceptor to send the token on every request
apiClient.interceptors.request.use((config) => {
    if (typeof window !== 'undefined') {
        const sessionId = localStorage.getItem('fallback_cart_id');
        if (sessionId) {
