...
export const trackPageView = async (path: string, visitorId: string): Promise<void> => {
    try {
        await apiClient.post('/api/public/analytics/ping', { path, visitorId });
    } catch (err: any) {
        // Log errors only in development to keep production console clean
        if (process.env.NODE_ENV === 'development') {
            console.error("Analytics ping failed:", err.message);
        }
    }
};
// ==============================================
