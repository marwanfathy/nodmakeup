...
const KikoProductCard: FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <div className="nod-product-card">
      <Link href={`/product/${product.slug}`} className="nod-product-card__main-link">
        <div className="nod-product-card__image-wrapper">
          
          {/* 1. SALE BADGE (P0 Priority: Clear Value Prop) */}
          {isOnSale && discountPercentage > 0 && (
            <span className="nod-badge-sale">Save {discountPercentage}%</span>
          )}
          
          {/* Keep Bestseller if needed, but Sale is usually more important for conversion */}
          {!isOnSale && product.isBestseller && (
            <div className="nod-product-card__bestseller-tag">
              <span>BESTSELLER</span>
            </div>
          )}

          <img
            src={mediaUrl(defaultVariant.imageUrl) || '/default-image.png'}
            alt={product.name}
        ...
        <div className="nod-product-card__details">
          ...
          <h3 className="nod-product-card__name">{product.name}</h3>
          <p className="nod-product-card__description">{product.shortDescription}</p>
          
          {/* 3. PRICE VISIBILITY (P1 Priority: Typography) */}
          <div className="nod-product-card__price">
            {isOnSale && (
              <span className="nod-product-card__price--original">
                EGP {originalPrice.toLocaleString()}
              </span>
            )}
            <span className={`nod-product-card__price--current ${isOnSale ? 'sale-color' : ''}`}>
      ...
      <div className="nod-product-card__variant-wrapper">
        {otherVariantsCount > 0 && (
          <Link href={`/product/${product.slug}`} className="nod-product-card__variant-selector">
      ...
      <div className="nod-product-card__cta-container">
        <button
          ...
          className={`nod-product-card__cta-button ${addState}`}
